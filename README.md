...
- Confirm the item description does not repeat a list of ATS platform names. Google rejected the
  0.5.9 listing for keyword spam, so uploading a correct binary without correcting that listing
...
- Run `npm run build` and inspect the verifier output for the package version and SmartRecruiters
  content-script match before uploading the zip.
...
**Out (deliberately):** the extension never creates third-party accounts, never fills SSN / driver's license / background-check consent, never checks a legal-agreement or accuracy-certification box, and never clicks Submit unless the student explicitly opted in per session and let the on-button countdown run. All AI generation stays on the backend; the extension is a client.

**Password fields — narrow exception (2026-07-23, reverses the 2026-07-03 email-only scope).** On a Workday *create-account* form the extension fills the password, derived per tenant from a device-local salt so the same account can be signed into again later. No password is ever stored at rest. It re-fills that password on a sign-in form **only** for an account the extension itself provisioned under the salt still in place; an account created by hand, created before this feature, or created on another device is never typed into, because submitting a wrong password is what locks a student out. The extension still never clicks Create Account and never completes email verification. Nothing here changes the account-creation boundary itself: the student, in their own session, is still the one who creates the account.
